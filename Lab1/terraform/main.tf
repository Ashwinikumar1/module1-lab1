...
resource "google_cloud_run_v2_service" "backend_service" {
  ...
  template {
    containers {
      image = "${var.region}-docker.pkg.dev/${local.project_id}/${google_artifact_registry_repository.app_repo.repository_id}/backend:latest"
      ports {
        container_port = 8080
      }
      env {
...
# Allow IAM invocation ONLY for IAP service account requests
...
# Reserved Static Global IP for Load Balancer
